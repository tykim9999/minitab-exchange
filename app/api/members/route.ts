...
export async function POST(req: Request) {
  ...
  try {
    ...
    // 선택된 책의 재고 확인
    const bookStockResults = await sql`SELECT books_quantity AS stock FROM Books WHERE books_id = ${members_book_id}`;
    ...
    if (bookStock <= 0) {
     ...
     VALUES (${members_name}, ${members_phone_number}, ${members_company}, ${members_message}, NULL, ${members_book_id}, ${members_winner_status})
     RETURNING *
   `;

    // 회원 정보가 성공적으로 저장된 후 책 재고를 감소
    await sql`UPDATE Books SET books_quantity = books_quantity - 1 WHERE books_id = ${members_book_id}`;
